...
import { Calendar, Clock, User, Phone, Mail, Briefcase, Send, CheckCircle2, Shield, Zap, Star, Users } from 'lucide-react'
...
export default function ConsultancyPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => { 
    e.preventDefault()
    setSubmitted(true)
    setTimeout(() => { setSubmitted(false); setForm({ name: '', email: '', phone: '', company: '', date: '', time: '', message: '' }) }, 4000)
  }

  const packages = [
    { name: 'Basic Audit', price: 'GHS 2,000', features: ['Security assessment', 'Basic report', '1 consultation'], icon: <Shield size={28} />, gradient: 'from-cyan-500 to-blue-600' },
    { name: 'Professional', price: 'GHS 5,000', features: ['Full security audit', 'Detailed report', '3 consultations', 'Action plan'], icon: <Zap size={28} />, gradient: 'from-purple-500 to-pink-600' },
    { name: 'Enterprise', price: 'GHS 15,000', features: ['Complete audit', 'Full report', 'Unlimited consultations', 'Implementation', '12-month monitoring'], icon: <Star size={28} />, gradient: 'from-orange-500 to-red-600' },
  ]
    ...
    <div className="min-h-screen bg-[#020617] text-white pt-28 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
          ...
          <p className="mt-4 text-xl text-gray-400">Strategic technology advisory for your business</p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-3 gap-6 mb-16">
          {packages.map((pkg, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <div className={`inline-flex p-4 rounded-2xl bg-gradient-to-br ${pkg.gradient} mb-4`}>{pkg.icon}</div>
              <h3 className="text-xl font-bold mb-2">{pkg.name}</h3>
              <p className="text-3xl font-black text-orange-400 mb-4">{pkg.price}</p>
              <div className="space-y-2 mb-6">
                {pkg.features.map(f => <div key={f} className="flex items-center gap-2 text-sm text-gray-400"><CheckCircle2 size={14} className="text-green-400" /> {f}</div>)}
              </div>
